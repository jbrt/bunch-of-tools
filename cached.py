...
def cached(func=None, timeout: int=60, redis_attribute: str='cache', key_prefix: str=None):
    """
    ...
    :param redis_attribute: (str) the name of the redis client attribute
    ...
    if not func:
        return partial(cached,
                       timeout=timeout,
                       redis_attribute=redis_attribute,
    ...
    def decorated_function(service, *args, **kwargs):
        ...
        # Get the Redis client store in redis_attribute
        redis = getattr(service, redis_attribute)
